function c = tom_multiprod(a, b, idA, idB)
%TOM_MULTIPROD  Multiplying 1-D or 2-D subarrays contained in two N-D arrays.
%   C = tom_multiprod(A,B) is equivalent  to C = tom_multiprod(A,B,[1 2],[1 2])
%   C = tom_multiprod(A,B,[D1 D2]) is eq. to C = tom_multiprod(A,B,[D1 D2],[D1 D2])
%   C = tom_multiprod(A,B,D1) is equival. to C = tom_multiprod(A,B,D1,D1)
%
%   tom_multiprod performs multiple matrix products, with array expansion (AX)
...
%         C = tom_multiprod(A, B, DA1, [DB1 DB2]) contains the products of the 
...
%             C = tom_multiprod(A, B, DA1, DB1) returns products of scalars by 
%             vectors, or vectors by scalars or scalars by scalars.
%      (b) If SIZE(A, DA1) == SIZE(B, DB1), 
%             C = tom_multiprod(A, B, [0 DA1], [DB1 0]) or 
%             C = tom_multiprod(A, B, DA1, DB1) virtually turns the vectors
...
%             C = tom_multiprod(CONJ(A), B, [0 DA1], [DB1 0]).
%      (c) Without limitations on the length of the vectors in A and B,
%             C = tom_multiprod(A, B, [DA1 0], [0 DB1]) turns the vectors
...
%             C = tom_multiprod(CONJ(A), B, [DA1 0], [0 DB1]).
%
...
%      X*10, or []-10). Similarly, in tom_multiprod, the purpose of AX is to
...
% 
%   tom_multiprod is a generalization for N-D arrays of the matrix
